...
package com.suneee.demo.scan.camera;
...
public final class CameraManager {
	...
	public PlanarYUVLuminanceSource buildLuminanceSource(byte[] data, int width, int height) {
		...
		//cp.add (用于解决java.lang.IllegalArgumentException。Crop rectangle does not fit within image data.)
		...
		if (cropWidth > width){
			cropWidth = width;
			cropLeft = 0;
		}else if(cropLeft + cropWidth > width){
		...
		if (cropHeight > height){
			cropHeight = height;
			cropTop = 0;
		}else if(cropTop + cropHeight > height){
		...
		case PixelFormat.YCbCr_422_SP:
			return new PlanarYUVLuminanceSource(data, width, height, cropLeft, cropTop, cropWidth, cropHeight);
		default:
