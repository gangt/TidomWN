 ...
 */
public abstract class HAbsListView extends AdapterView<ListAdapter> implements TextWatcher, ViewTreeObserver.OnGlobalLayoutListener, Filter.FilterListener,
	...
	public interface OnScrollListener {
		 ...
		 */
		public void onScroll(HAbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount);
	}
	...
	public HAbsListView(Context context, AttributeSet attrs) {
		this(context, attrs, 0 /*
								 * com.android.internal.R.attr.absListViewStyle
								 * bysong
								 */);
	}
	...
	public HAbsListView(Context context, AttributeSet attrs, int defStyle) {
		...
		// bysong
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.HAbsListView, defStyle, 0);
		...
		mDrawSelectorOnTop = a.getBoolean(R.styleable.HAbsListView_drawSelectorOnTop, false);

		// boolean stackFromBottom =
		// a.getBoolean(R.styleable.AbsListView_stackFromBottom, false);
		...
		// a.getBoolean(R.styleable.AbsListView_textFilterEnabled, false);
	...
	static class SavedState extends BaseSavedState {
		...
		public String toString() {
			return "AbsListView.SavedState{" + Integer.toHexString(System.identityHashCode(this)) + " selectedId=" + selectedId + " firstId=" + firstId
					+ " viewTop=" + viewTop + " viewLeft=" + viewLeft + " position=" + position + " height=" + height + " filter=" + filter + "}";
		}
	...
	protected boolean setFramex(int left, int top, int right, int bottom) {
		...
		changed = ReflectUtil.view_setFrame(HAbsListView.class.getSuperclass().cast(View.class), HAbsListView.class.getSuperclass().getSuperclass()
				.getSuperclass(), left, top, right, bottom);
	...
	void updateScrollIndicators() {
		...
		if (mScrollDown != null) {
			...
			if (!canScrollDown && count > 0) {
				...
				// canScrollDown = child.getBottom() > mBottom -
				// mListPadding.bottom;
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		if (clipToPadding) {
			...
			canvas.clipRect(scrollX + getPaddingLeft(), scrollY + getPaddingTop(), scrollX + getRight() - getLeft() - getPaddingRight(), scrollY + getBottom()
	...
	private boolean performLongPress(final View child, final int longPressPosition, final long longPressId) {
		...
		if (mOnItemLongClickListener != null) {
			handled = mOnItemLongClickListener.onItemLongClick(HAbsListView.this, child, longPressPosition, longPressId);
		}
		if (!handled) {
	...
	public boolean showContextMenuForChild(View originalView) {
		...
		if (longPressPosition >= 0) {
			...
			if (mOnItemLongClickListener != null) {
				handled = mOnItemLongClickListener.onItemLongClick(HAbsListView.this, originalView, longPressPosition, longPressId);
			}
			if (!handled) {
		...
		case MotionEvent.ACTION_DOWN: {
			...
			if (!mDataChanged) {
				if ((mTouchMode != TOUCH_MODE_FLING) && (motionPosition >= 0) && (getAdapter().isEnabled(motionPosition))) {
					...
					// User clicked on whitespace, or stopped a fling. It is a
					// scroll.
					createScrollingCache();
					mTouchMode = TOUCH_MODE_SCROLL;
					mMotionCorrection = 0;
				...
				if (x != mLastX) {
					...
					if (motionView != null) {
						...
						if (motionView.getLeft() != mMotionViewNewLeft) {
							// We did not scroll the full amount. Treat this
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		if (DEBUG) {
			...
			// Log.d(TAG, "onInterceptTouchEvent. mFirstPosition: " +
			// mFirstPosition);
		}
	...
	private class FlingRunnable implements Runnable {
		...
		public void run() {
			...
			if (delta > 0) {
				...
				// Don't fling more than 1 screen
				// delta = Math.max(-(getHeight() - mPaddingBottom - mPaddingTop
				// - 1), delta);
			...
			mClearScrollingCache = new Runnable() {
				public void run() {
					if (mCachingStarted) {
						...
						// if ((mPersistentDrawingCache &
						// PERSISTENT_SCROLLING_CACHE) == 0) {
	...
	boolean resurrectSelection() {
		...
		if (toPosition >= firstPosition && toPosition < firstPosition + childCount) {
			...
			if (selectedLeft < childrenLeft) {
				...
				selectedLeft = childrenBottom - selected.getMeasuredWidth() - getHorizontalFadingEdgeLength();
			}
		} else {
	...
	private void createTextFilter(boolean animateEntrance) {
		if (mPopup == null) {
			...
			mTextFilter = (EditText) layoutInflater.inflate(0/*
	...
	class RecycleBin {
		...
		void scrapActiveViews() {
			...
			for (int i = 0; i < count; ++i) {
				...
				if (victim != null) {
					int whichScrap = ((HAbsListView.LayoutParams) victim.getLayoutParams()).viewType;
