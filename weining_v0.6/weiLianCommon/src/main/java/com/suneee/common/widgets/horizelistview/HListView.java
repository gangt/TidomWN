...
public class HListView extends HAbsListView {
	...
	public HListView(Context context, AttributeSet attrs, int defStyle) {
		...
		final Drawable d = a.getDrawable(/* com.android.internal. */R.styleable.HListView_dividerHV);
		if (d != null) {
	...
	private void removeFixedViewInfo(View v, ArrayList<FixedViewInfo> where) {
		Iterator<FixedViewInfo> iterator = where.iterator();
		while (iterator.hasNext()) {
			FixedViewInfo info = iterator.next();
			if (info.view == v) {
				iterator.remove();
				break;
			}
	...
	void fillGap(boolean down) {
		...
		if (down) {
			final int startOffset = count > 0 ? getChildAt(count - 1).getRight() + mDividerWidth : getListPaddingLeft();
			fillDown(mFirstPosition + count, startOffset);
			correctTooHigh(getChildCount());
		} else {
			final int startOffset = count > 0 ? getChildAt(0).getLeft() - mDividerWidth : getWidth() - getListPaddingRight();
	...
	private View moveSelection(View oldSel, View newSel, int delta, int childrenLeft, int childrenRight) {
		...
		final int topSelectionPixel = getTopSelectionPixel(childrenLeft, fadingEdgeLength, selectedPosition);
		final int bottomSelectionPixel = getBottomSelectionPixel(childrenLeft, fadingEdgeLength, selectedPosition);
		...
		if (delta > 0) {
			...
			if (newSel != null) {
				...
				// was selected
				sel = makeAndAddView(selectedPosition, newSel.getRight(), true, mListPadding.top, true);
			} else {
				// If (A) was not on screen and so did not have a view, position
				// it above the oldSel (B)
				sel = makeAndAddView(selectedPosition, oldSel.getRight(), false, mListPadding.top, true);
			}
	...
	private void setupChild(View child, int position, int x, boolean flowDown, int childrenTop, boolean selected, boolean recycled) {
		...
		if (p == null) {
			p = new HAbsListView.LayoutParams(ViewGroup.LayoutParams.FILL_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT, 0);
		}
		p.viewType = mAdapter.getItemViewType(position);
	...
	private ArrowScrollFocusResult arrowScrollFocused(final int direction) {
		...
		if (selectedView != null && selectedView.hasFocus()) {
			...
			if (direction == View.FOCUS_LEFT) {
				...
				final int ySearchPoint = (selectedView != null && selectedView.getLeft() > listLeft) ? selectedView.getLeft() : listLeft;
				...
				final int listRight = getWidth() - mListPadding.right - (bottomFadingEdgeShowing ? getArrowScrollPreviewLength() : 0);
				final int ySearchPoint = (selectedView != null && selectedView.getRight() < listRight) ? selectedView.getRight() : listRight;
				mTempRect.set(0, ySearchPoint, 0, ySearchPoint);
			}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see android.view.View#findViewWithTag(String) First look in our
	...
	public static class SavedState extends BaseSavedState {
		...
		public String toString() {
			return "ListView.SavedState{" + Integer.toHexString(System.identityHashCode(this)) + " checkState=" + checkState + "}";
		}
