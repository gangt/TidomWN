...
public class FaceLockActivity extends BaseActivity implements Camera.PreviewCallback, GLSurfaceView.Renderer, SurfaceTexture.OnFrameAvailableListener, WebServiceUtil.OnDataListener {
        ...
        mHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (faces != null) {
                    ...
                    if (faces.length > 0) {
                        if (faces.length > 1) {
                            ToastUtils.showCenterToast(FaceLockActivity.this, "请保持摄像头中只有一张人脸");
                            return;
                        }
                            ...
                            if (saveFile) {
                                ...
                                try {
                                    ...
                                    if (!TextUtils.isEmpty(faceToken)) {
                                        // 获取到脸部的faceToken
                                        WebServiceUtil.request(Constants.BIND_AUTH_ACCOUNT_FACETOKEN, "json", FaceLockActivity.this);
                                    } else {
                                        // 获取脸部的faceToken失败
                                        ToastUtils.showCenterToast(FaceLockActivity.this, "获取脸部信息失败，重新获取");
                                        saveFile = true;
                                    }
        ...
        try {
            ...
            if (status == 1) {
                if (result.getBoolean("data")) {
                    SharedPrefUtils.saveStringData(this, Constants.FACE_TOKEN, faceToken);
                    ToastUtils.showCenterToast(FaceLockActivity.this, "脸部解锁设置成功");
                    timeHandle.removeMessages(0);
                    finish();
                }
            } else {
                ToastUtils.showCenterToast(this, message);
            }
        } catch (Exception e) {
    ...
    public String onGetParamDataString(String mode) {
        ...
        try {
            jsonObject.put("sessionId", SharedPrefUtils.getSessionId(this));
            jsonObject.put("authType", "FACEDIS");
            jsonObject.put("facktoken", faceToken);
        } catch (Exception e) {
            e.printStackTrace();
        }
