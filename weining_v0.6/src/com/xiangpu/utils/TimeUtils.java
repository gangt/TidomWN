...
/**
 * Created by fangfumin on 2017/7/21.
 */
...
public class TimeUtils {
     ...
     * @param starTime 开始时间
     * @param endTime  结束时间
     * @param boolean  返回类型 是否大于
     * @return 返回时间差
     */
    public static boolean getTimeCompare(String starTime, String endTime) {
        boolean time = false;
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

        try {
            Date starParse = dateFormat.parse(starTime);
            Date endParse = dateFormat.parse(endTime);

            time = endParse.getTime() > starParse.getTime();
