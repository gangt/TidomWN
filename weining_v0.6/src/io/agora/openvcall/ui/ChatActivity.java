...
public class ChatActivity extends BaseActivity implements AGEventHandler, View.OnClickListener,
    ...
    private volatile boolean mVideoMuted = false;//视频打开关闭模式

    private volatile boolean mAudioMuted = false;//声音静音模式开启关闭

    private volatile boolean mPreview = false;
    ...
    private static ChatResultCallback chatResultCallback;

    public static void setChatResultCallback(ChatResultCallback callback) {
        chatResultCallback = callback;
    }

    public interface ChatResultCallback {
        void onChatSuccess();

        void onChatFaile(String errorMsg);
    }

    private CommonDialog chatFinishDialog = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        chatFinishDialog = new CommonDialog(this);
        chatFinishDialog.setOnResultListener(new CommonDialog.OnResultListener() {
            @Override
    ...
    protected void initUIandEvent() throws Exception {
        ...
        mGridVideoViewContainer = (GridVideoViewContainer) this.findViewById(R.id.grid_video_view_container);
        mGridVideoViewContainer.setVisibility(View.VISIBLE);

        //水平布局
        LinearLayoutManager layoutManager = new LinearLayoutManager(this);
        layoutManager.setOrientation(LinearLayoutManager.HORIZONTAL);
        mGridVideoViewContainer.setLayoutManager(layoutManager);
        ...
        mGridVideoViewContainer.setItemEventHandler(new VideoViewEventListener() {
            ...
            public void onItemDoubleClick(View v, Object item) {
                ...
                //根据选择的muid 如果是0就显示本地，相反就远程。
                UserStatusData user = (UserStatusData) item;
                int uid = (user.mUid == 0) ? config().mUid : user.mUid;

                if (mLayoutType == LAYOUT_TYPE_DEFAULT && mUidsList.size() != 1) {
                    switchToSmallVideoView(uid);//小图像
                    showLongToast("切换switchToSmallVideoView");
                } else {
                    switchToDefaultVideoView();//全屏显示
                    showLongToast("切换switchToDefaultVideoView");
                }
            }
        });

        //加载本地头像，用到 surfaceV,和对应的uid
        SurfaceView surfaceV = RtcEngine.CreateRendererView(getApplicationContext());
        rtcEngine().setupLocalVideo(new VideoCanvas(surfaceV, VideoCanvas.RENDER_MODE_ADAPTIVE, 0));
        //rtcEngine().setupRemoteVideo(new VideoCanvas(surfaceV, VideoCanvas.RENDER_MODE_ADAPTIVE, 0));
        ...
        int uid = config().mUid;
        LogUtil.d("channelName: " + channelName + " uid: " + uid);
        worker().joinChannel(channelName, uid);
        ...
        //计时开始
        this.findViewById(R.id.bottom_action_end_call).setOnClickListener(ChatActivity.this);
    ...
    public void onChagePreview(View view) {
        muteLocalVideoStream();
    }

    public void muteLocalVideoStream() {
        RtcEngine rtcEngine = rtcEngine();
        rtcEngine.muteLocalVideoStream(mPreview);
        mPreview = !mPreview;
    }

    private boolean speek = true;

    public void setDefaultAudioRoutetoSpeakerphone(View view) {
        RtcEngine rtcEngine = rtcEngine();
        rtcEngine.setEnableSpeakerphone(speek);

        ImageView imageView = (ImageView) view;
        if (speek) {
            imageView.setBackgroundResource(R.drawable.skin_icon_video_select);
        } else {
            imageView.setBackgroundResource(R.drawable.skin_icon_video_hf_jump);
        }
        speek = !speek;
    }
    ...
    public void onVoiceMuteClicked(View view) {
        ...
        if (!mAudioMuted) {
            iv.setColorFilter(getResources().getColor(R.color.agora_blue), PorterDuff.Mode.MULTIPLY);
            iv.setBackgroundResource(R.drawable.skin_icon_btn_mute_off_normal);
        } else {
            iv.clearColorFilter();
            iv.setBackgroundResource(R.drawable.skin_icon_btn_mute_forbidden);
        }
    }
    ...
    @Override
    public void onJoinChannelSuccess(String channel, final int uid, final int elapsed) {
    ...
    private void switchToDefaultVideoView() {
        ...
        mGridVideoViewContainer.invalidate();
    }
    ...
    private void bindToSmallVideoView(final int exceptUid) {
        ...
        if (mSmallVideoViewAdapter == null) {
            ...
            SurfaceView surfaceVLocal = mUidsList.remove(config().mUid);
            ...
            mSmallVideoViewAdapter = new SmallVideoViewAdapter(this, config().mUid, exceptUid, mUidsList, new VideoViewEventListener() {
                ...
                public void onItemDoubleClick(View v, Object item) {
                    ...
                    int configuid = config().mUid;
                    if(uid == config().mUid && mLayoutType != LAYOUT_TYPE_DEFAULT){
                        //switchToDefaultVideoView();//全屏显示
            ...
            mUidsList.put(config().mUid,surfaceVLocal);
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if (keyCode == event.KEYCODE_BACK) {
            // 捕获按下返回键事件
            chatFinishDialog.show();
            chatFinishDialog.setTvContent("是否结束此次视频？");
            return true;
        }
    ...
    public String onGetParamDataString(String mode) {
        ...
        if(mode != null && mode.equals(Constants.VIDEO_AUDIO_HANG_UP)){//挂断会议
            try {
                ...
                json.put("numbers", (mUidsList.size() > 0 ? mUidsList.size()-1:0) );
